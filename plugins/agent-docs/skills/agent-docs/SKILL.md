...
name: agent-docs
description: >
  Hält die Agent-Doku (CLAUDE.md/AGENTS.md, .claude/rules) am Code — legt neu an, wenn keine
  da ist, sonst Diff-Sync nach Code-Änderungen. Löschen-bevorzugt, strenges Add-Gate;
  0 Änderungen ist ein gültiges Ergebnis. Standard nur Vorschlag, geschrieben wird erst mit
  `--fix`. AUSLÖSER: Code geändert, "Doku syncen", "kürzen"/"weniger Doku", oder ein Repo ohne

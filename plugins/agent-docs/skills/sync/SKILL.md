...
argument-hint: "[base-branch] [pfad]"
...
`$ARGUMENTS` — alle optional, frei kombinierbar, Reihenfolge egal:

- Token existiert als Pfad im Repo → **Subtree-Fokus** (nur dort Kandidaten suchen; Cross-Refs zu anderen Files trotzdem prüfen).
- Token ist Branch-/Ref-Name → **Branch-Sync**: Diff gegen `git merge-base <ref> HEAD` statt Working Tree.
- Leer → Working Tree + Änderungen dieser Session.
...
1. **Snapshot.** `git status --short` + `git diff --stat` (Branch-Sync: Diff gegen die Merge-Base laut Argument). Wenn keine Code-Änderungen → "Nothing to sync." Stop.
